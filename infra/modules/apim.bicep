...
var innerPolicy = '${ipFilterXml}<rate-limit calls="${rateLimitCalls}" renewal-period="${rateLimitPeriod}" /><quota calls="${quotaCalls}" renewal-period="${quotaPeriod}" /><validate-content unspecified-content-type-action="prevent" max-size="262144" size-exceeded-action="prevent" errors-variable-name="requestBodyErrors"><content type="application/json" validate-as="json" action="prevent" /></validate-content><set-header name="X-Correlation-ID" exists-action="override"><value>@(context.Request.Headers.GetValueOrDefault(&quot;X-Correlation-ID&quot;, context.RequestId.ToString()))</value></set-header><set-backend-service base-url="{{la-bmwc-ingest-url}}" />'
